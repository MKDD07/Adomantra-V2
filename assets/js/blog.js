    ...
    const htmlChunks = await Promise.all(blogs.map(async (blog, index) => {
      ...
      return `
                <div class="col-12 col-md-6 col-lg-4 wow fadeInUp" data-wow-delay="${0.1 * (index % 3)}s">
                    <div class="features-item blog-section-5__item h-100 d-flex flex-column" style="padding: 24px; border-radius: 32px;">
                        ...
                        <h3 class="blog-section-5__title original-black" style="font-size: 28px; line-height: 1.1; margin-bottom: 20px;">
                        ...
                        <div class="blog-thumb-wrap" style="flex-grow: 1; overflow: hidden; border-radius: 24px;">
                            <a href="${url}">
                                <img src="${image}" alt="${displayTitle}" loading="lazy" 
                                     style="width: 100%; aspect-ratio: 1/1; object-fit: cover; transition: transform 0.8s ease;"
                        ...
                        <div class="d-flex align-items-center justify-content-between mt-4">
                            ...
                            <div class="blog-section-5__icon">
                                <a href="${url}" style="width: 45px; height: 45px; display: flex; align-items: center; justify-content: center; background: var(--primary); color: white; border-radius: 50%;">
    ...
    const htmlChunks = await Promise.all(featuredBlogs.map(async (blog, index) => {
      ...
      return `
                <div class="features-item p-0 project-area7__card section-item overflow-hidden wow fadeInUp" data-wow-delay="${0.2 * index}s" style="border-radius: 32px; background: white;">
                    ...
                    <div class="project-area7__card-thumb" style="overflow: hidden; border-radius: 0 0 32px 32px;">
                        <img src="${image}" alt="${title}" style="aspect-ratio: 4/3; object-fit: cover; width: 100%; transition: transform 0.8s ease;">
                        <a href="${url}" class="project-area7__card-btn" style="border-radius: 0; text-align: center; padding: 15px;">See Details</a>
                    </div>
                </div>
            `;
    }));
