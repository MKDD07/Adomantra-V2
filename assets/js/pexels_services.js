 ...
 */
async function fetchPexelsImage(query, randomize = false) {
    const cacheKey = `img_${query.toLowerCase().replace(/\s+/g, '_')}_${randomize ? 'rand' : 'single'}`;

    // 1. Check local cache first
    try {
        const cachedBlob = await AdomantraCache.get(cacheKey);
        if (cachedBlob) {
            console.log('🖼️ Loading from cache:', query);
    ...
    try {
        ...
        if (data.photos && data.photos.length > 0) {
            ...
            console.log('💾 Caching new image:', query);
...
function initGenericPexelsImages() {
    ...
    $targets.each(function () {
        var $el = $(this);
        var query = $el.data('pexels-query') || 'digital marketing';
